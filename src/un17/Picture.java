...
public class Picture extends SimplePicture {
    ...
    void negate() {
        ...
        for (Pixel[] r : pixels) {
            for (Pixel c : r) {
                int sum=c.getRed()+c.getGreen()+c.getGreen();
                c.setRed(sum/3);
                c.setGreen(sum/3);
                c.setBlue(sum/3);
            }
    ...
    void mirrorArms() {
        ...
        for (int col = 0; col < pixels[0].length; col++) {
            for (int row = 170; row < 200; row++) {
                leftPixel = pixels[row][col];
                rightPixel = pixels[pixels.length-1-row+100][col];
                rightPixel.setColor(leftPixel.getColor());
            }
        }
  }
    ...
    public void edgeDetection2(int edgeDist) {
        ...
        for (int row = 0; row < pixels.length; row++) {
            for (int col = 0;
                ...
                rightPixel = pixels[row][col + 1];
                rightColor = rightPixel.getColor();
                float[] colors = Color.RGBtoHSB((int) Math.pow((10 * leftPixel.colorDistance(rightColor)), 1), 255 - (int) Math.pow((10 * leftPixel.colorDistance(rightColor)), .5), 0, null);
